...
**"Medium 난이도를 정복하면, 코딩 테스트의 90%는 해결된다."**

*   **황금 비율:** `Easy(20%)` : `Medium(70%)` : `Hard(10%)`
*   **Easy:** 문법 확인 및 워밍업 (모르면 문법 공부 필요)
*   **Medium:** **학습의 핵심.** 유형별 표준 패턴 체화 (시간 투자의 70% 집중)
*   **Hard:** 사고력 확장 (Medium 2개를 섞어놓은 형태가 많음)

### ⏳ 학습 규칙 (Rule)
1.  **Time Boxing:** 한 문제당 고민 시간은 **최대 20분**.
2.  **Look & Learn:** 20분 내에 아이디어가 안 떠오르면, 과감히 **정답(Solution)과 로직을 먼저 학습**한다.
3.  **Implement:** 코드를 안 보고 **직접 다시 구현**해본다.
